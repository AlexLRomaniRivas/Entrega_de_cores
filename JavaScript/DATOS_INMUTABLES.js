...
const y3 = ({id}) => id % 3 == 0;
const py3 = videojuegos.filter(y3)


//  Un conjunto de videojuegos que pertenecen al género “acción-RPG”.
const a2 = videojuegos.filter(juego => juego.genero.includes("acción-RPG"));

//  Un conjunto de videojuegos que tienen más de un género.
const a3 = videojuegos.filter(juego => juego.genero.includes(" "));

//  Una lista con los nombres de los videojuegos.
const a4 = videojuegos.map(juego=>juego.nombre)

//  Una lista con los nombres de los videojuegos con un número de identificación superior a 19.
const e5 = videojuegos.filter(juego =>juego.id >19).map(juego=>juego.nombre)

//  Una lista con los nombres de los videojuegos cuyo único género es “shooter”.
const e6 = videojuegos.filter(juego => juego.genero=="shooter").map(juego=>juego.nombre)

//  Una lista que contenga solo el primer género de todos los videojuegos cuyo segundo género es “aventura”.
const e7= videojuegos.filter(juego=>juego.genero.includes("-aventura")).map(juego=>juego.genero.slice(0,6))

//  Un conteo del número de videojuegos que son del género “party”.
const e8 = videojuegos.filter(juego=>juego.genero.includes("party")).length

//  Una lista con todos los videojuegos excepto aquellos cuyo número de identificación sea múltiplo de 5.
const e9 = videojuegos.filter(juego=> juego.id % 5 !== 0).map(juego=>juego.nombre)

//  Un lista con todos los videojuegos y para el videojuego con el número de identificación 5, se cambia su género por “otro”.
const e10 = [...videojuegos].map(juego => 
    juego.id ==5? {...juego, genero: "otro"} : juego
)
const e10s=videojuegos.filter(juego=>juego.id==5)